...
local state = settings.global[SETTING_MOD_NAME].value
...
local function on_mod_state_changed(event)
	if event.setting == SETTING_MOD_NAME then
		state = settings.global[event.setting].value
		reregister_events()

		if state then
			game.print({"mod-enabled", {"colon"}, {"mod-name." .. script.mod_name}})
			for _, lib in pairs(libraries) do
			...
			game.print({"mod-disabled", {"colon"}, {"mod-name." .. script.mod_name}})
			for _, lib in pairs(libraries) do
...
local register_events = function()
	events_when_on = {
		[defines.events.on_runtime_mod_setting_changed] = {_core = on_mod_state_changed}
	}
	events_when_off = {
		[defines.events.on_runtime_mod_setting_changed] = {_core = on_mod_state_changed}
	}
